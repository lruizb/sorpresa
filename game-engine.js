...
function updateJoystickInput() {
  ...
  if (degrees > -45 && degrees < 45) {
    ...
    player.inputVelocityX = CONFIG.PLAYER_MAX_SPEED;
  }
  else if (degrees > 135 || degrees < -135) {
    joystick.direction = -1;
    player.inputVelocityX = -CONFIG.PLAYER_MAX_SPEED * 0.5;
  }
  else {
    joystick.direction = 0;
    player.inputVelocityX = 0;
...
function generateLevel() {
  ...
  while (currentX < levelEndX) {
    ...
    if (structureCount < 3 && rand > 0.2 && rand < 0.5) {
      ...
      if (Math.random() < 0.6) {
        specialBlocks.push(new MysteryBlock(currentX, platformY - 40, 'coin'));
        specialBlocks.push(new MysteryBlock(currentX + 40, platformY - 40, 'mushroom'));
      }
...
function redimensionarCanvas() {
  ...
  joystick.centerX = canvas.width * 0.15;
  joystick.centerY = canvas.height * 0.75;
  jumpButton.x = canvas.width * 0.85;
  jumpButton.y = canvas.height * 0.75;
